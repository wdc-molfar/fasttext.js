    ...
    FastText = (function () {
        ...
        function FastText(options) {
            ...
            this.exec = function (name, command, params, options) {
                ...
                // console.log(command, params, _options)
        ...
        FastText.prototype.loadnn = function () {
            console.log("Loadnn", this._options)
            var self = this;
            return new Promise(function (resolve, reject) {
        ...
        /**
         * Load model for Print Word Vectors
         */
        FastText.prototype.loadPWV = function () {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (self._options.debug) console.log("loadPWV", args);
                self.child = self.exec('fasttext', self._options.bin, args, self._options.child);
                if (self._options.debug) self.child.stdout.pipe(process.stdout);

                self._printWordVectors("галя")
                    .then(res => {
                        return resolve(true);
                    })
                    .catch(error => {
        ...
        FastText.prototype._printWordVectors = function(data) {
            var self = this;
            return new Promise(function (resolve, reject) {
                // register callbacks
                var onErrorCallback = function (error) {
                ...
                var onDataCallback = function (_data) {
                    ...
                    let r ={
                        ...
                        query: data.splice(0,1)[0] 
                    }
                    r.value = data.map( d => Number.parseFloat(d)).filter(d => !Number.isNaN(d))
                    return resolve(r);
                }
                var onErrorDataAppendCallback = function (data) {
                ...
                if (self._options.printWordVectors.normalize) { // normalize and send
                    console.log("send ", FastText.normalize(data))
        ...
        FastText.prototype.printWordVectors = function(data) {
            data = data.split(" ").map( d => d.trim()).filter(d => d)
            return Promise.reduce(data, (res, d) => 
                this._printWordVectors(d)
                    .then( r => {
                        res.push(r)
                        return res
                    })
        ...
        /**
         * Load model for Print Sentence Vectors
         */
        FastText.prototype.loadPSV = function () {
            ...
            return new Promise(function (resolve, reject) {
                ...
                if (self._options.debug) console.log("loadPSV", args);
                self.child = self.exec('fasttext', self._options.bin, args, self._options.child);
                if (self._options.debug) self.child.stdout.pipe(process.stdout);

                self.printSentenceVectors("галя")
                    .then(res => {
                        return resolve(true);
                    })
                    .catch(error => {
        ...
        FastText.prototype._printSentenceVectors = function(sentence) {
            var self = this;
            return new Promise(function (resolve, reject) {
                // register callbacks
                var onErrorCallback = function (error) {
                ...
                var onDataCallback = function (_data) {
                    var data = _data.split(" ").map( d => Number.parseFloat(d)).filter(d => !Number.isNaN(d))
                    let r ={
                ...
                if (self._options.printSentenceVectors.normalize) { // normalize and send
                    return self.send(FastText.normalize(sentence));
                } else { // send raw
                    self.send(sentence);
                }
            });
        ...
        FastText.prototype.printSentenceVectors = function(data) {
            data = data.split("\n").map( d => d.trim())
            return Promise.reduce(data, (res, d) => 
                this._printSentenceVectors(d)
                    .then( r => {
                        res.push(r)
                        return res
                    })
        ...
        FastText.prototype.quantize = function (params) {
            var self = this;
            Util.mergeRecursive(this._options, params);
            return new Promise(function (resolve, reject) {
                self.prepareDataset({ filePath: self._options.trainFile, tmpFilePath: self._options.tmpFilePath })
