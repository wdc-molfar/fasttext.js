...
// console.log(root)
let p = path.resolve(root,`bin/${process.platform}/fasttext.exe`)
            // console.log("Executable: ",p)
            return p
            // process.chdir(root);
            // var binpath = resolve('.' + path.sep + 'bin' + path.sep + process.platform + path.sep + filename);
            // process.chdir(cdir);

            // if (fs.existsSync(binpath)) { // check local binary path
            //     return binpath;
            // }
            // else if (CLIUtil.commandExistsSync(filename)) { // check global binary path
            //     return filename;
            // }
            // return null;
        },
         ...
         */
        GetFolderPath: function (folder, filename) {
            var cdir = process.cwd();
        ...
        */
        ...
        /**
         * Check if a string has unicode characters (Double Bytes)
         */
        ...
        /**
         * Check if a strinh contains non latin chars
         */
        ...
        /**
         * Remove white spaces to left and to right
         * @param string str
         * @return string
         */
        trim: function (str) {
        ...
        */
        getTextLinesNormalized: function (text, diacritics = true, lower = true, latin = true, punct = true) {
            var lines = text.split(/\n/);
            lines = lines.map(line => this.removeAllPunctations(line, false, diacritics, lower, latin, punct));
            return lines;
        ...
        removeAllPunctations: function (text, newline = ' ', diacritics = true, lower = true, latin = true, punct = true) {
            text = this.trim(text);
            if (diacritics) text = this.removeDiacritics(text);
            if (lower) text = text.toLowerCase();
            if (newline) text = text.replace(/(?:\\[rn]|[\r\n]+)+/g, newline); // \r \n
            text = text.replace(/\t+/g, '\t').replace(/\t\s/g, ' ').replace(/\t/g, ' ') // tabs
            if (punct) {
                text = text
                    .replace(/(\s+[`'"‘])\w+\b(?!\2)/g, '') // contractions
                    .replace(/[\\+;:\?!\»\«\>\<\]\[\)\(,\.\‘'“”"…]/g, ' ') // punct
                    .replace(/[\.\.\.]/g, ' ') // ellipsis
            }
            if (latin) { // to latin
                var isUnicode = this.isDoubleByte(text);
                if (!isUnicode) { // not unicode
                    text = text.replace(/[^a-zA-Zà-úÀ-Úá-úÁ-ÚñÑ0-9ążśźęćńółæøåÆØßẞâàéèëêïîöôùüûœçåÅäÄöÖüÜяё]+/g, ' ') // remove all not latin1 chars
                }
            }
            return text;
         ...
         * @return Promise
         */
        promiseAll: function (items, block) {
            var promises = [];
